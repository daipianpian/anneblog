...
<script type="text/ecmascript-6">
	import {classifysifEditClassify, classifyUpdateClassify} from "../../../config/interface.js"
	export default {
        props: {
        },
        data() {
		...
		methods: {
			...
			editClassify() {
				const url = classifysifEditClassify
	            const _this = this
				let params = {
					classifyId: this.formDatas.id
				}
				fetch(url,params)
				.then(res =>{
					if(res.code == 10000){
			...
			onSubmit(formDatas){
				this.$refs[formDatas].validate((valid) => {
					if (valid) {
						const url = classifyUpdateClassify
			            const _this = this
			            let params = {
			            	classifyId: this.formDatas.id,
